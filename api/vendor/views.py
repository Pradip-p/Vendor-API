...
class ProvincesViewset(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        serializer_class = ProvincesSerializers(queryset, many=True)
        return Response(serializer_class.data)
    ...
    def retrieve(self, request, pk=None):
        ...
        if id is not None:
            vendor = Provinces.objects.get(pk=id)
            serializer_class = ProvincesSerializers(vendor)
            return Response(serializer_class.data)
    ...
    def update(self,pk, request):
        ...
        id=pk
        vendor = Provinces.objects.get(pk=id)
        serializer_class = ProvincesSerializers(vendor, data=request.data)
        if serializer_class.is_valid():
            serializer_class.save()
            return Response({"msg":"Data Created"}, status=status.HTTP_201_CREATED)
    ...
    def partial_update(self,pk, request):
        ...
        id=pk
        vendor = Provinces.objects.get(pk=id)
        serializer_class = ProvincesSerializers(vendor, data=request.data,partial=True)
        if serializer_class.is_valid():
            serializer_class.save()
